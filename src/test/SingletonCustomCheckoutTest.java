...
public class SingletonCustomCheckoutTest extends BaseSetup {
    ...
    void testSingletonViaGetInstance() {
        CustomCheckoutClient customCheckoutClient1 = CustomCheckoutClient.getInstance(clientId,
                clientSecret, clientVersion, env);
        CustomCheckoutClient customCheckoutClient2 = CustomCheckoutClient.getInstance(clientId,
                clientSecret, clientVersion, env);
    ...
    void testSingletonWithDiffParameters() {
        ...
        PhonePeException phonePeException = assertThrows(PhonePeException.class,
                () -> CustomCheckoutClient.getInstance("clientId2",
    ...
    void testMultipleClientSingleAuthCall() {
        ...
        CustomCheckoutClient customCheckoutClient2 = CustomCheckoutClient.getInstance(clientId,
                clientSecret, clientVersion, env);
        CustomCheckoutClient customCheckoutClient3 = CustomCheckoutClient.getInstance(clientId,
                clientSecret, clientVersion, env);
        CustomCheckoutClient customCheckoutClient4 = CustomCheckoutClient.getInstance(clientId,
                clientSecret, clientVersion, env);

        final String url = CustomCheckoutConstants.PAY_API;
        addStubForPostRequest(url, getHeaders(), pgPaymentRequest, HttpStatus.SC_OK, Maps.newHashMap(),
                pgPaymentResponse);
