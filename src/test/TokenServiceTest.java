...
public class TokenServiceTest extends BaseSetup {
    ...
    void testFailFirstFetch() throws JsonProcessingException, InterruptedException {
        PhonePeResponse phonePeResponse = PhonePeResponse.builder()
                ...
                .errorCode("errorCode")
                .message("Bad Request")
                .data(Collections.singletonMap("errorDescription", "Client Authentication Failure"))
                .build();
        TokenService.setOAuthResponse(null);
        wireMockServer.stubFor(post(urlPathMatching(authUrl)).willReturn(aResponse()
                .withStatus(400)
                .withHeader("Content-Type", "application/json")
                .withBody(objectMapper.writeValueAsString(phonePeResponse))));
    ...
    void testOAuthFetchToken() throws JsonProcessingException {
        ...
        String actual = standardCheckoutClient.getTokenService()
                .getAuthToken();
    ...
    void testTokenRefresh() {
        ...
        doReturn(oAuthResponse).when(spyTokenService)
                .fetchTokenFromPhonePe();
        String setToken = spyTokenService.getAuthToken();  //sets the token
        String secondCall = spyTokenService.getAuthToken(); //notices token is invalid and fetched new token
    ...
    void testTokenUseCached() {
        ...
        String noRefresh = spyTokenService.getAuthToken(); //tries to fetch new token but token is valid
    ...
    void testTokenCachedUseWithCurrentTime() {
        ...
        doReturn(currentTime).when(spyTokenService)
                .getCurrentTime();
        ...
        doReturn(currentTime + 1).when(spyTokenService)
                .getCurrentTime();

        setToken = spyTokenService.getAuthToken();  //tries to fetch new token as token is invalid now
        setToken = spyTokenService.getAuthToken();  //tries to fetch new token as token is invalid now
        setToken = spyTokenService.getAuthToken();  //tries to fetch new token as token is invalid now
        setToken = spyTokenService.getAuthToken();  //tries to fetch new token as token is invalid now
    ...
    void testWhenUnauthorizedTokenForPay() {
        ...
        addStubForPostRequest(url, getHeaders(), standardCheckoutPayRequest, HttpStatus.SC_UNAUTHORIZED,
                Maps.newHashMap(), phonePeResponse);

        final UnauthorizedAccess unauthorizedAccess = assertThrows(UnauthorizedAccess.class,
        ...
        Assertions.assertEquals("O-Bearer accessToken", standardCheckoutClient.getTokenService()
                .formatCachedToken());
    ...
    void testWhenUnauthorizedTokenForOrder() throws JsonProcessingException {
        ...
        addStubForGetRequest(url, ImmutableMap.of(StandardCheckoutConstants.ORDER_DETAILS, "false"),
                getHeaders(), HttpStatus.SC_UNAUTHORIZED, ImmutableMap.of(), phonePeResponse);
        ...
        addStubForGetRequest(url, ImmutableMap.of(StandardCheckoutConstants.ORDER_DETAILS, "false"),
                getHeaders(), HttpStatus.SC_OK, ImmutableMap.of(), orderStatusResponse);
    ...
    void testFirstFetchWorksSecondFetchFailsSendsBackOldToken() throws JsonProcessingException {
        ...
        wireMockServer.stubFor(post(urlPathMatching(authUrl))
        ...
        String setToken = standardCheckoutClient.getTokenService()
                .getAuthToken();
        ...
        String shouldReceiveOldToken1 = standardCheckoutClient.getTokenService()
                .getAuthToken();
        String shouldReceiveOldToken2 = standardCheckoutClient.getTokenService()
                .getAuthToken();
        String shouldReceiveOldToken3 = standardCheckoutClient.getTokenService()
                .getAuthToken();
