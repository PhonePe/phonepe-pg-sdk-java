...
public class SubscriptionSetupTest extends BaseSetupWithOAuth {
    ...
    void testSetupViaUpiIntentSuccess() {
        ...
        PgPaymentResponse expected = PgPaymentResponse.builder()
                .state("PENDING")
                .orderId("OMOxxx")
                .build();
        Map<String, String> headers = getHeadersForSubscription();

        addStubForPostRequest(url, headers, setupRequest, HttpStatus.SC_OK, Maps.newHashMap(), expected);
    ...
    void testSetupFailure() {
        ...
        addStubForPostRequest(url, headers, setupRequest, HttpStatus.SC_BAD_REQUEST, Maps.newHashMap(),
                phonePeResponse);

        final PhonePeException phonePeException = assertThrows(PhonePeException.class,
                () -> subscriptionClient.setup(setupRequest));
