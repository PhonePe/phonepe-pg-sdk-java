...
public class SubscriptionClient extends BaseClient {
    ...
    private SubscriptionClient(String clientId, String clientSecret, Integer clientVersion, Env env,
            boolean shouldPublishEvents) {
        super(clientId, clientSecret, clientVersion, env, shouldPublishEvents);
        this.eventPublisher.send(BaseEvent.buildInitClientEvent(FlowType.SUBSCRIPTION,
                EventType.SUBSCRIPTION_CLIENT_INITIALIZED));
        this.prepareHeaders();
    }
     ...
     */
    public static synchronized SubscriptionClient getInstance(final String clientId, final String clientSecret,
    ...
    public static synchronized SubscriptionClient getInstance(final String clientId, final String clientSecret,
        ...
        if (Objects.isNull(client)) {
            client = new SubscriptionClient(clientId, clientSecret, clientVersion, env, shouldPublishEvents);
            return client;
        }

        String requestedSha256 = CommonUtils.calculateSha256(clientId, clientSecret, clientVersion, env,
                shouldPublishEvents, FlowType.SUBSCRIPTION);
        String existingSha = CommonUtils.calculateSha256(client.getCredentialConfig()
                        .getClientId(),
                client.getCredentialConfig()
                        .getClientSecret(),
                client.getCredentialConfig()
                        .getClientVersion(), client.getEnv(), client.isShouldPublishEvents(), FlowType.SUBSCRIPTION);

        if (Objects.equals(requestedSha256, existingSha)) {
            return client;
        }
        throw new PhonePeException(
                "Cannot re-initialize SubscriptionClient. Please utilize the existing client object with required "
                        + "credentials");
    }
     ...
     *
     * @param request Request built using PgPaymentRequest Builder's. 1. Payment via UPI_INTENT  -> use
     *                PgPaymentRequest.SubscriptionSetupUpiIntentBuilder() 2. Payment via UPI_COLLECT -> use
    ...
    public PgPaymentResponse setup(PgPaymentRequest request) {
        ...
        try {
            PgPaymentResponse response = requestViaAuthRefresh(HttpMethodType.POST, request, url, null,
                    new TypeReference<PgPaymentResponse>() {}, headers);
            this.eventPublisher.send(BaseEvent.buildSubscriptionSetupEvent(EventState.SUCCESS, request, url,
                    EventType.SETUP_SUCCESS));
            return response;
        } catch (Exception exception) {
            this.eventPublisher.send(BaseEvent.buildSubscriptionSetupEvent(EventState.FAILED, request, url,
                    EventType.SETUP_FAILED, exception));
            throw exception;
        }
    }
    ...
    public PgPaymentResponse notify(PgPaymentRequest request) {
        ...
        try {
            ...
            this.eventPublisher.send(
                    BaseEvent.buildSubscriptionNotifyEvent(EventState.SUCCESS, request, url, EventType.NOTIFY_SUCCESS));
            return response;
        } catch (Exception exception) {
            this.eventPublisher.send(
                    BaseEvent.buildSubscriptionNotifyEvent(EventState.FAILED, request, url, EventType.NOTIFY_FAILED,
                            exception));
            throw exception;
        }
    }
    ...
    public SubscriptionRedeemResponseV2 redeem(String merchantOrderId) {
        ...
        try {
            ...
            SubscriptionRedeemResponseV2 response = requestViaAuthRefresh(HttpMethodType.POST, request, url, null,
                    new TypeReference<SubscriptionRedeemResponseV2>() {}, headers);
            this.eventPublisher.send(BaseEvent.buildSubscriptionRedeemEvent(EventState.SUCCESS, merchantOrderId, url,
                    EventType.REDEEM_SUCCESS));
            return response;
        } catch (Exception exception) {
            this.eventPublisher.send(BaseEvent.buildSubscriptionRedeemEvent(EventState.FAILED, merchantOrderId, url,
                    EventType.REDEEM_FAILED, exception));
            throw exception;
        }
    }
     ...
     * @return SubscriptionStatusResponseV2 which contains the entire status of the given subscription ID
     */
    @SneakyThrows
    public SubscriptionStatusResponseV2 getSubscriptionStatus(String merchantSubscriptionId) {
        final String url = String.format(SubscriptionConstants.SUBSCRIPTION_STATUS_API, merchantSubscriptionId);
        try {
            SubscriptionStatusResponseV2 response = requestViaAuthRefresh(HttpMethodType.GET, null, url,
                    null, new TypeReference<SubscriptionStatusResponseV2>() {}, headers);
            this.eventPublisher.send(BaseEvent.buildSubscriptionStatusEvent(EventState.SUCCESS, merchantSubscriptionId,
                    url, EventType.SUBSCRIPTION_STATUS_SUCCESS));
            return response;
        } catch (Exception exception) {
            this.eventPublisher.send(BaseEvent.buildSubscriptionStatusEvent(EventState.FAILED, merchantSubscriptionId,
                    url, EventType.SUBSCRIPTION_STATUS_FAILED, exception));
            throw exception;
        }
    }
    ...
    public OrderStatusResponse getOrderStatus(String merchantOrderId) {
        ...
        try {
            OrderStatusResponse response = requestViaAuthRefresh(HttpMethodType.GET, null, url,
                    null, new TypeReference<OrderStatusResponse>() {}, headers);
            this.eventPublisher.send(BaseEvent.buildOrderStatusEvent(EventState.SUCCESS, merchantOrderId, url,
                    FlowType.SUBSCRIPTION, EventType.ORDER_STATUS_SUCCESS));
            return response;
        } catch (Exception exception) {
            this.eventPublisher.send(BaseEvent.buildOrderStatusEvent(EventState.FAILED, merchantOrderId, url,
                    FlowType.SUBSCRIPTION, EventType.ORDER_STATUS_FAILED, exception));
            throw exception;
        }
    }
    ...
    public void cancelSubscription(String merchantSubscriptionId) {
        ...
        try {
            requestViaAuthRefresh(HttpMethodType.POST, null, url,
                    null, new TypeReference<Void>() {}, headers);
            this.eventPublisher.send(
                    BaseEvent.buildSubscriptionCancelEvent(EventState.SUCCESS, merchantSubscriptionId, url,
                            EventType.CANCEL_SUCCESS));
        } catch (Exception exception) {
            this.eventPublisher.send(
                    BaseEvent.buildSubscriptionCancelEvent(EventState.FAILED, merchantSubscriptionId, url,
                            EventType.CANCEL_FAILED, exception));
            throw exception;
        }
    }
     ...
     * @return OrderStatusResponse which contains the status of the particular transaction in the paymentDetails
     * attribute
     */
    @SneakyThrows
    public OrderStatusResponse getTransactionStatus(String transactionId) {
        final String url = String.format(SubscriptionConstants.TRANSACTION_STATUS_API, transactionId);
        try {
            OrderStatusResponse response = requestViaAuthRefresh(HttpMethodType.GET, null, url, null,
                    new TypeReference<OrderStatusResponse>() {}, headers);
            this.eventPublisher.send(BaseEvent.buildTransactionStatusEvent(EventState.SUCCESS, transactionId, url,
                    FlowType.SUBSCRIPTION, EventType.TRANSACTION_STATUS_SUCCESS));
            return response;
        } catch (Exception exception) {
            this.eventPublisher.send(BaseEvent.buildTransactionStatusEvent(EventState.FAILED, transactionId, url,
                    FlowType.SUBSCRIPTION, EventType.TRANSACTION_STATUS_FAILED, exception));
            throw exception;
        }
    }
    ...
    public RefundResponse refund(RefundRequest refundRequest) {
        ...
        try {
            ...
            this.eventPublisher.send(
                    BaseEvent.buildRefundEvent(EventState.SUCCESS, refundRequest, url, FlowType.SUBSCRIPTION,
                            EventType.REFUND_SUCCESS));
            return refundResponse;
        } catch (Exception exception) {
            this.eventPublisher.send(
                    BaseEvent.buildRefundEvent(EventState.FAILED, refundRequest, url, FlowType.SUBSCRIPTION,
    ...
    public RefundStatusResponse getRefundStatus(String refundId) {
        ...
        try {
            ...
            this.eventPublisher.send(
                    BaseEvent.buildRefundStatusEvent(EventState.SUCCESS, refundId, url, FlowType.SUBSCRIPTION,
                            EventType.REFUND_STATUS_SUCCESS));
            return refundStatusResponse;
        } catch (Exception exception) {
            this.eventPublisher.send(
                    BaseEvent.buildRefundStatusEvent(EventState.FAILED, refundId, url, FlowType.SUBSCRIPTION,
    ...
    public CallbackResponse validateCallback(String username, String password, String authorization,
        ...
        try {
            ...
            this.eventPublisher.send(
                    BaseEvent.buildCallbackSerializationFailedEvent(EventState.FAILED, FlowType.SUBSCRIPTION,
